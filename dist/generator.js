...
function resolveType(fieldSchema) {
    if (fieldSchema.type)
...
// Template functions
function tplPackageJson(appName) {
...
function tplApiClient(endpoints) {
...
function extractResources(endpoints, components) {
    ...
    for (const ep of endpoints) {
        ...
        if (ep.method === 'POST') {
            res.canCreate = true;
            let schema = ep.requestBody?.content?.['application/json']?.schema;
            if (schema)
            ...
            for (const [fieldName, fieldSchema] of Object.entries(props)) {
                ...
                const isOwnerField = (t === 'integer' || t === 'number') &&
                    (fieldName === 'user' || fieldName.endsWith('_user'));
                if (isOwnerField) {
                    res.ownerField = fieldName;
                    continue;
                }
                const extra = fieldSchema.extra;
                const fk = extra?.foreign_key;
                const fkTable = fk?.to;
                res.fields.push({
                    name: fieldName,
...
function renderResourceEntityFile(res) {
    const R = res.name;
    const label = res.label;
    const typeName = singular(label);
    const fkResources = [...new Set(res.fields.filter(f => f.fkResource).map(f => f.fkResource))];
    const fkOptionStates = fkResources.map(r => `  const [${r}Options, set${cap(r)}Options] = useState<{id:number;[k:string]:unknown}[]>([]);`).join('\n');
    const fkOptionLoaders = fkResources.map(r => `  useEffect(() => { if (session.authenticated) getItems<{id:number;[k:string]:unknown}>('${r}').then(set${cap(r)}Options).catch(() => {}); }, [session]);`).join('\n');
    const newStateFields = res.fields.map(f => {
        if (f.type === 'boolean')
            return `  const [new${cap(f.name)}, setNew${cap(f.name)}] = useState<boolean>(false);`;
        if (f.type === 'number')
            return `  const [new${cap(f.name)}, setNew${cap(f.name)}] = useState<string>('');`;
        return `  const [new${cap(f.name)}, setNew${cap(f.name)}] = useState('');`;
    }).join('\n');
    const editStateFields = res.fields.map(f => {
        if (f.type === 'boolean')
            return `  const [edit${cap(f.name)}, setEdit${cap(f.name)}] = useState<boolean>(false);`;
        if (f.type === 'number')
            return `  const [edit${cap(f.name)}, setEdit${cap(f.name)}] = useState<string>('');`;
        return `  const [edit${cap(f.name)}, setEdit${cap(f.name)}] = useState('');`;
    }).join('\n');
    const createPayload = res.fields.map(f => {
        if (f.type === 'number')
            return `      ${f.name}: Number(new${cap(f.name)}),`;
        if (f.type === 'boolean')
    ...
    const updatePayload = res.fields.map(f => {
        if (f.type === 'number')
            return `      ${f.name}: Number(edit${cap(f.name)}),`;
        if (f.type === 'boolean')
    ...
    const resetFields = res.fields.map(f => {
        if (f.type === 'boolean')
            return `      setNew${cap(f.name)}(false);`;
        if (f.type === 'number')
    ...
    const tdCells = columns.map(c => {
        if (c === 'id')
            return `<td style={td}>{(item as Record<string,unknown>).id as number}</td>`;
...
${res.fields.map(f => {
        if (f.type === 'boolean')
            return `      setEdit${cap(f.name)}(Boolean(item.${f.name}));`;
...
// App.tsx template
function tplAppTsx(title, resources) {
...
export async function generateApp(schema, outputDir, appName, _apiUrl) {
